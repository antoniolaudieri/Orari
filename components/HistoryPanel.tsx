            ...
            <ul className="space-y-4">
              {sortedEntries.map((entry, index) => (
                <li key={entry.id} className="bg-slate-900/50 p-4 rounded-lg flex flex-col sm:flex-row items-center gap-4 ring-1 ring-slate-700 opacity-0 animate-slideInUp" style={{ animationDelay: `${index * 75}ms`}}>
                  <div className="w-24 h-16 flex-shrink-0 bg-black/30 rounded-md overflow-hidden">
                    <img src={`data:${entry.mimeType};base64,${entry.imageData}`} alt="Anteprima storico" className="w-full h-full object-contain" />
                  </div>
